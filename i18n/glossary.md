...
- **각주 라벨**: 용어 각주 라벨(`[^vla]` 등)은 기계적 식별자 — 절대 번역·현지화하지
  않는다. 정의 블록(`[^라벨]: …`)의 개수·순서는 4개 언어 동일, 정의 헤드의 대시는
  전 언어 `—` 하나로 통일. 같은 라벨 각주가 여러 페이지에 있고 원문이 동일하면
  역어도 페이지 간 동일하게 쓴다(원문이 페이지별로 다르면 각자 따라간다 — 예: `[^dr]`,
  `[^sys]`). 라벨이 어긋나도 `mkdocs build --strict`는 통과하고 본문에 `[^라벨]`
  리터럴이 그대로 노출된다 — 자동 게이트가 없으므로 수동 대조 필수.
  정의 블록 상단의 `<!-- 용어 각주 -->` 주석도 기계적 마커 — 4개 언어에서 번역하지 않고 그대로 유지한다.
