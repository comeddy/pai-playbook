...
aws-samples(+awslabs) 조직에서 Physical AI 관련 신규 리포를 찾아 필러 관련 자산으로 등재하는 표준 절차. 2026-07-31 수동 조사(PR #4)를 루틴화한 것.
...
2. **Dedup**: 이미 등재된 리포는 제외 —
   `grep -rho "github.com/aws-\(samples\|labs\)[a-zA-Z0-9/-]*" docs/*.md | grep -v '\.\(en\|zh\|ja\)\.' | sort -u`
3. **제외 기준** (`gh api repos/<owner>/<repo> --jq '[.archived, .pushed_at, .topics]'`):
   아카이브 / 마지막 push 18개월 초과 / RoboMaker 등 폐기 서비스 종속 / 실무 관련성 없는 장난감 데모.
4. **실체 검증**: 후보별 README 확인(무엇을 배포하고 무엇이 WIP/데모인지 — 캐비앗 문구 그대로 발췌), URL `curl -sIL -o /dev/null -w '%{http_code}'` 200 확인.
5. **편입**: 가장 맞는 필러의 **🔗 관련 자산** 불릿 목록에 기존 형식으로 추가 —
   `- [이름 — 한 줄 정체](URL) — aws-samples. 기능 요약. ⚠️ 성숙도 캐비앗(README 자기 선언 우선)`.
...
- **README 자기 선언을 넘어서는 주장 금지** — "데모"라고 쓰인 리포를 역량처럼 소개하지 않는다.
