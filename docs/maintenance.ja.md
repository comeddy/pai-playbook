...
```
_owner: {名前} · updated: {YYYY-MM} · volatility: 高/中/低_
```

- `owner`: 未定であれば `未定 ⚠️` と表示し、**負債として残します**（隠しません）。
...
**⚙️ 自動化済み**: バッジは手動で付けません。CI(`scripts/check_staleness.py`)が **ビルド直前に自動注入** し、**毎週月曜の cron 再デプロイ** がプッシュなしでもバッジを更新します。ページの `updated`/`volatility` メタデータが欠落するとビルドが失敗します — メタデータこそが契約です。
...
---
_owner: {名前} · updated: {YYYY-MM} · volatility: 高/低_
```
...
graph TD
    ...
    F -- 通過 --> PR["③ 昇格 — 担当ピラー owner が標準テンプレートで編入<br>· 成熟度ラベル + 出典等級を付与<br>· 揮発性情報は折りたたみブロックに隔離<br>· owner/updated を記入"]
