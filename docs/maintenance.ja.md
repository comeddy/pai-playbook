...
- **エグゼクティブページ（exec/exec-guide）原則**: 新しい技術的主張は禁止 — pillar/radar の検証判定のエグゼクティブ言語への翻訳のみを載せます。

**用語脚注の慣例（継続）**: コンテンツを追加・更新する際、SA がすぐに理解できない用語は同じ変更の中で `[^ラベル]` 脚注として処理します — 既存のラベルがあれば再利用し、新しい用語はページ最下部の `<!-- 용어 각주 -->` ブロックに「**用語** — 1～2文の説明」形式で追加します。検証済みの公式動画があれば定義の末尾に 🎥 リンクを付けます（マージ前に oEmbed[^oembed] レスポンスでタイトル・チャンネルを照合）。マーカーは本文の初出位置のみに置き、見出し・mermaid ブロックには禁止。ラベルは機械的識別子のため翻訳禁止、4言語に同一に適用します（ラベル・URL 同一）— 詳細ルールは `i18n/glossary.md` を参照。
...
| **playbook 管理者（Youngjin）** | staleness バッジ、構造の一貫性、四半期レビュー |
...
1. ~~全項目 owner 未定~~ → **全ページ owner: Youngjin 指定完了** — P1～P5・index・radar（2026-07）、decisions・maintenance（2026-08）、playbook 管理者の役割（2026-08）。owner・役割の負債はすべて解消しました。
...
[^staleness]: **staleness（鮮度低下）** — 文書・データが最後のレビュー以降古くなり、実際と食い違っていく度合いです。この playbook はページごとの変動性等級で許容期間（1/3/6 か月）を定め、超過するとバッジを自動付与します。
[^oembed]: **oEmbed** — URL ひとつでそのコンテンツのタイトル・作成者などの埋め込み情報を JSON で返す標準 API です。動画リンクが実在するか、タイトル・チャンネルが合っているかを、公開前に機械的に検証するのに使います。
