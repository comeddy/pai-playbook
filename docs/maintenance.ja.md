...
**用語脚注の慣例（継続）**: コンテンツを追加・更新する際、SA がすぐに理解できない用語は同じ変更の中で `[^ラベル]` 脚注として処理します — 既存のラベルがあれば再利用し、新しい用語はページ最下部の `<!-- 용어 각주 -->` ブロックに「**用語** — 1～2文の説明」形式で追加します。検証済みの公式動画があれば定義の末尾に 🎥 リンクを付けます（マージ前に oEmbed レスポンスでタイトル・チャンネルを照合）。マーカーは本文の初出位置のみに置き、見出し・mermaid ブロックには禁止。ラベルは機械的識別子のため翻訳禁止、4言語に同一に適用します（ラベル・URL 同一）— 詳細ルールは `i18n/glossary.md` を参照。
