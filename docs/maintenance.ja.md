...
**顧客事例**: (あれば。なければ「事例待ち」)
**➡️ SA 次のアクション**: (デモ/ワークショップ/資産へのリンク — 必ず埋める)
...
## Slack → playbook 昇格パイプライン

```
[Slack/ブログ/論文/デモ]  候補発生
        │
        ▼
   ① キャプチャ ──── 指定チャンネル + 絵文字リアクション（例: 📌）で候補を収集
   │            または GitHub イシューフォーム "📌 Playbook 候補の提報"（THE FILTER チェックリスト内蔵）
        │
        ▼
   ② フィルタ ──── 2.5 ゲート（4 項目中 2 項目以上？）
        │
        ├─ 未達 ──► Radar ページに一行（ラベル + なぜ待機 + 昇格条件）
        │
        └─ 通過 ──► ③ 昇格
                        │
                        ▼
                  担当ピラー owner が標準テンプレートで編入
                  · 成熟度ラベル + 出典等級を付与
                  · 揮発性情報は折りたたみブロックに隔離
                  · owner/updated を記入
                        │
                        ▼
                  ④ 作成前の自己点検（下記）を通過後にマージ
```
