...
## 🆕 最新扫描流入（2026-09-16 · 一手验证完成 2026-07-21）
...
| **[DWMP](https://arxiv.org/abs/2609.12347)**（结合身体动力学模型与视觉世界模型的双结构人形机器人行走策略） | 🔵 Research | ✨ **关注**：具体展示了"世界模型驱动行走"该如何实现的架构方案 —— 将本体感觉（关节·姿态）预测模型与深度视觉世界模型结合，蒸馏为轻量级 student 策略，在 Unitree G1 实机上以随机障碍物布局测试，通过率达 0.95/0.80/0.70（蒸馏基线为 0.80/0.60/0.65）—— 与 Radar 中 DreamZero/GR00T N2/Genie 3 的世界模型主线角度不同（学术架构提案）<br>⏳ **待定**：arXiv 预印本（2026-09-11）`[4]` —— 单一论文，未标明机构隶属，消融实验细节（置信区间等）不足，无独立验证。⚠️ 本次运行环境的出网限制导致未能执行人工 curl 200 检查（详见提交说明/issue） | 同行评审 + 独立复现 + 确认机构隶属 |
...
| **[XPENG IRON 人形机器人量产线](https://www.xpeng.com/pressroom/news/01a080371029a057bc8e8a02a2c6012b)**（汽车级自动化生产线，76 DOF·搭载自研 Turing AI 芯片 3 颗） | ⚪ 路线图 | ✨ **关注**：中国电动车企 XPENG 在广州启动人形机器人专用生产线，IRON 在线上自主行走下线 —— 将整车品质管控体系直接应用于机器人量产，是与 Tesla Optimus 量产延迟形成对比的竞争硬件赛道（与现代·BD Atlas、UBTech 角度不同：整车企业主导的自动化量产线）<br>⏳ **待定**：XPENG 官方公布（2026-09-08，xpeng.com pressroom）`[4]` —— 仅核心工序 80%+ 自动化及单台下线演示成功，年底目标的实际量产尚未开始（中国上市及海外交付目标为 2027 年）。无自主性能数据或独立验证。⚠️ 本次运行环境的出网限制导致未能执行人工 curl 200 检查（详见提交说明/issue） | 确认年底量产启动 + 自主性能独立验证 |
