...
## 🆕 最新扫描流入（2026-09-10 · 一手验证完成 2026-07-21）
...
| **[AgiBot World 2026](https://github.com/OpenDriveLab/AgiBot-World)**（计划分 5 阶段陆续开源的大规模真实环境机器人数据集） | 🔵 Research | ✨ **关注**：中国 AgiBot 用 AgiBot G2 机器人在商业空间·家庭等真实环境中采集的数据，按模仿学习→强化学习→世界模型等 5 个研究方向分阶段开源 —— 连失败-恢复轨迹都做了标注的机器人数据管道参考案例（IROS 2025 最佳论文入围·IEEE TRO 2026 发表的 AgiBot-World 后续项目）<br>⏳ **待定**：GitHub 官方仓库（`OpenDriveLab/AgiBot-World`，代码 Apache-2.0）`[4]` —— 目前只公开第 1 阶段（模仿学习），数据集本身许可证为 CC BY-NC-SA 4.0，限制商用。其余 4 个阶段尚未公开，无量化规模·独立验证 | 公开全部 5 阶段 + 确定商业许可条款 |
