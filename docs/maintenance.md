...
- **임원 페이지(exec/exec-guide) 원칙**: 새로운 기술 주장 금지 — pillar/radar 검증 판정의 임원 언어 번역만 싣는다.

**용어 각주 관례 (지속)**: 내용을 편입·갱신할 때 SA가 바로 이해하기 어려운 용어는 같은 변경 안에서 `[^라벨]` 각주로 처리한다 — 기존 라벨이 있으면 재사용, 새 용어면 페이지 최하단 `<!-- 용어 각주 -->` 블록에 "**용어** — 1~2문장 설명" 형식으로 추가한다. 검증된 공식 영상이 있으면 정의 끝에 🎥 링크(머지 전 oEmbed[^oembed] 응답으로 제목·채널 대조). 마커는 본문 첫 등장 위치에만, 헤딩·mermaid 블록 금지. 라벨은 기계적 식별자라 번역 금지, 4개 언어에 동일 적용(라벨·URL 동일) — 상세 규칙은 `i18n/glossary.md`.
...
[^staleness]: **staleness(신선도 저하)** — 문서·데이터가 마지막 검토 이후 낡아 실제와 어긋나 가는 정도. 이 playbook은 페이지별 변동성 등급으로 허용 기간(1/3/6개월)을 정하고, 초과하면 배지를 자동 부착한다.
[^oembed]: **oEmbed** — URL 하나로 해당 콘텐츠의 제목·작성자 등 임베드 정보를 JSON으로 돌려주는 표준 API. 영상 링크가 실존하는지, 제목·채널이 맞는지를 게시 전에 기계적으로 검증하는 데 쓴다.
