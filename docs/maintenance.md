...
**용어 각주 관례 (지속)**: 내용을 편입·갱신할 때 SA가 바로 이해하기 어려운 용어는 같은 변경 안에서 `[^라벨]` 각주로 처리한다 — 기존 라벨이 있으면 재사용, 새 용어면 페이지 최하단 `<!-- 용어 각주 -->` 블록에 "**용어** — 1~2문장 설명" 형식으로 추가한다. 검증된 공식 영상이 있으면 정의 끝에 🎥 링크(머지 전 oEmbed 응답으로 제목·채널 대조). 마커는 본문 첫 등장 위치에만, 헤딩·mermaid 블록 금지. 라벨은 기계적 식별자라 번역 금지, 4개 언어에 동일 적용(라벨·URL 동일) — 상세 규칙은 `i18n/glossary.md`.
