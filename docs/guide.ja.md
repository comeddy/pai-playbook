...
流入項目の一次ソース確認（公式発表·論文原文·ライセンス照合）と昇格判断は**人（owner）**が行います。自動スキャンはもっともらしい誤り（例：存在しない製品世代名）を作りうるため、検証なしには昇格されません。通過すれば owner が[標準テンプレート](maintenance.md#標準テンプレート)で該当ピラーに編入し、Radar から除去します。全手順は[昇格パイプライン](maintenance.md#playbook-昇格パイプライン)を参照してください。
