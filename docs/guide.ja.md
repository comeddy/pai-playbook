...
```
[候補の発見]
 ├─ 🤖 週次自動スキャン (毎週月 02:00 UTC — arXiv·ウェブ)
 ├─ 💬 SA タレコミ (昇格パイプライン)
 └─ 🔍 手動調査
        │
        ▼
[Radar キュー]  ← すべて「未検証 [4]」ラベル、顧客提案での使用禁止
        │
        ▼
[一次ソース検証]  ← 人 + 検証エージェント (公式発表·論文原文と照合)
        │
        ▼
[THE FILTER]  ⓐ production 検証  ⓑ AWS マッピング  ⓒ 実際の問い合わせ  ⓓ GA
        │                                          │
        │ 2 つ以上充足                             │ 未達
        ▼                                          ▼
[ピラー本文へ昇格]                        [Radar に一行で維持]
 (owner が標準テンプレートで)              (昇格条件を明記)
        │
        ▼
[掲載された後も]
 ├─ ⏳ 鮮度の自動監視 (volatility 別に 1/3/6 か月を超過するとバッジ)
 ├─ 🌐 4 言語同期 (ko 原本 → ko_hash → en/zh/ja)
 └─ ✅ strict ビルドゲート (アンカー·リンク検証) → GitHub Pages へ自動デプロイ
```
