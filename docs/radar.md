...
## 🆕 최신 스캔 유입 (2026-09-08 · 1차 검증 완료 2026-07-21)
...
| **[AGIBOT WORLD 2026 Theme 3: Reinforcement Learning](https://www.agibot.com/article/231/detail/88.html)** (성공·실패·인간 개입까지 담은 실세계 RL 데이터셋) | 🔵 Research | ✨ **주목**: [pillar-1](pillar-1.md)에 이미 있는 AgiBot World(100만+ 궤적, CC BY-NC-SA 비상업)의 후속 테마 — 성공 시연뿐 아니라 자율 정책 롤아웃의 실패·오류·인간 개입 교정까지 담아 모방학습을 넘어 RL 축으로 데이터 플라이휠을 넓히는 신호(로봇 데이터 파이프라인 각도)<br>⏳ **대기**: AGIBOT 공식 발표(agibot.com, 2026-08-07) `[4]` — 1차 릴리스 14개 태스크(이더넷 케이블 삽입·키로 문 잠금해제·바코드 라벨 부착·데스크램프 포장 등), 9,638궤적·164시간+(전체 공개분 11,430궤적). 여전히 **CC BY-NC-SA 4.0 비상업**(pillar-1과 동일 라이선스 리스크), 독립 벤치마크·실배포 검증 없음 | 상업 라이선스 전환/명확화 + 독립 벤치마크 재현 |
| **[Figure × Nscale $35억 컴퓨트 파트너십](https://www.prnewswire.com/news-releases/nscale-and-figure-sign-strategic-partnership-to-power-the-next-generation-of-physical-ai-302868918.html)** (NVIDIA Vera Rubin GPU 최대 10만 대, Nscale의 Figure 지분투자 동반) | ⚪ 로드맵 | ✨ **주목**: 경쟁 휴머노이드사가 대규모 훈련 컴퓨트를 AWS가 아닌 유럽계 네오클라우드와 묶는 첫 사례 — AWS SA에게는 경쟁사 컴퓨트 소싱 지형과 Physical AI 훈련 인프라 수요 규모를 보여주는 참고 신호<br>⏳ **대기**: Figure·Nscale 공식 발표(2026-09-03) `[4]` — 초기 약정 $35억(최대 10만 대 Vera Rubin GPU), $60억+ 확장 의향 + Nscale의 Figure 지분투자 동반. 배치는 2027년 하반기(텍사스 Barstow) 목표로 로드맵 단계, 실가동 0 | 실제 배치 개시 확인 + AWS 대응 사례 비교 근거 |
