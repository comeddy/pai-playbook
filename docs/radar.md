...
## 🆕 최신 스캔 유입 (2026-09-21 · 1차 검증 완료 2026-07-21)
...
| **[Strands Robots](https://strandsagents.com/blog/robots-working-together-model-hardware-standard-strands-robots/)** (AWS 오픈소스 에이전트 프레임워크 Strands Agents의 로봇·물리 하드웨어 자연어 제어 확장, Zenoh 메시 네트워킹 + AWS IoT Core 플릿 연동) | 🟡 Preview | ✨ **주목**: AWS 자체 오픈소스 에이전트 프레임워크 Strands Agents가 자연어로 로봇을 직접 제어하는 확장을 공개 — Robot()/Simulation() 인스턴스가 자동으로 Zenoh 메시 피어가 되어 로컬 네트워크에서 서로 발견·제어하고, AWS IoT Core로 지역 분산 플릿까지 연동, ROS 2 그래프도 그대로 관측·명령 가능 — Radar 🔗 에이전트/연결 축(MCP for robotics, AgentCore Payments/Registry)에 AWS 자사 로보틱스 에이전트 사례가 처음 추가됨<br>⏳ **대기**: strands-labs/robots 오픈소스 저장소(2026-08, v0.5.x) + strandsagents.com 공식 블로그 `[4]` — docs.aws.amazon.com에 GA 서비스로 문서화되지 않은 커뮤니티/OSS 단계, 함께 발표된 "Model Hardware Standard"는 제한적 리서치 프리뷰. ⚠️ 이번 실행 환경 egress 제한으로 curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | AWS 공식 서비스 GA 문서화 + 실제 로봇 플릿 프로덕션 사례 공개 |
| **[WholeBodyWAM](https://arxiv.org/abs/2609.16644)** (사전학습 월드-액션 모델을 whole-body controller 기반 전신 제어로 확장하는 휴머노이드 loco-manipulation 프레임워크) | 🔵 Research | ✨ **주목**: 조작 중심으로 사전학습된 월드-액션 모델(WAM)의 지식을 유지한 채 이종 전신 제어기(whole-body controller) 시맨틱을 접지시켜, 보행까지 포함한 전신 협응 loco-manipulation으로 일반화 — 시뮬 종합 성공률 91.9%로 Radar 🔬 World-action models 축(DreamZero→GR00T N2)을 조작에서 전신 제어로 확장하는 최신 학계 사례<br>⏳ **대기**: arXiv 프리프린트(2026-09-17) `[4]` — 자체 시뮬 벤치마크뿐, 실기 검증·peer-review·독립 재현 없음. ⚠️ 이번 실행 환경 egress 제한으로 curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | 실기(sim-to-real) 검증 + peer-review + 독립 재현 |
