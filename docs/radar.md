...
## 🆕 최신 스캔 유입 (2026-09-19 · 1차 검증 완료 2026-07-21)
...
| **[TANGO](https://arxiv.org/abs/2609.09158)** (관절공간 액션을 직접 예측하는 전신 제어형 휴머노이드 내비게이션 VLA) | 🔵 Research | ✨ **주목**: 2D 웨이포인트가 아니라 29자유도 관절공간 액션을 직접 예측해, 장애물 많은 실내에서 숙이기·옆걸음·넘어가기까지 언어 지시에 따라 스스로 결정하는 whole-body VLA — 자체 시뮬 데이터 파이프라인(Plan-Edit-Track, 6.46만 궤적)으로 학습 후 Unitree G1에 zero-shot sim-to-real[^s2r] 전이 성공, VLA와 sim-to-real 두 축을 동시에 다루는 학계 사례<br>⏳ **대기**: arXiv 프리프린트(2026-09-08, UC Berkeley·Peking Univ·Tsinghua·HKU·Princeton 공동) `[4]` — 자체 벤치마크(성공률·SPL·충돌률)뿐, peer-review·독립 재현 없음. 데이터 파이프라인·모델·체크포인트는 오픈소스 예정이나 아직 미공개. ⚠️ 이번 실행 환경 egress 제한으로 curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | peer-review + 오픈소스 공개 후 독립 재현 |
| **[GE-Act 2.0](https://arxiv.org/abs/2609.05588)** (AgiBot Genie 팀의 제로베이스 사전학습 월드-액션 모델) | 🔵 Research | ✨ **주목**: 기존 월드-액션 모델(WAM)들이 사전학습된 영상 생성모델을 그대로 물려받는 것과 달리, 생성·액션 컴포넌트를 처음부터 매니퓰레이션 데이터로 학습 — Radar의 World-action models(DreamZero→GR00T N2, Genie 3) 축에 중국 AgiBot발 오픈 연구 사례 추가<br>⏳ **대기**: arXiv 프리프린트(2026-09-04, AgiBot Genie Team·NUS LV-Lab·BUAA 공동) `[4]` — 자체 벤치마크(시뮬·실기 매니퓰레이션 성공률)뿐, peer-review·독립 재현 없음. ⚠️ 이번 실행 환경 egress 제한으로 curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | peer-review + 독립 재현 + 오픈소스 공개 |
...
| **[Robot Data Factory (RDF)](https://arxiv.org/abs/2609.16705)** (로봇 "경험" 수집·검증을 표준화하는 미션→태스크→스킬→에피소드→데이터셋→벤치마크 계층 인프라, MBZUAI) | 🔵 Research | ✨ **주목**: Radar 유입 중 처음으로 "물리 AI 데이터 파이프라인" 축을 모델·파트너십이 아니라 데이터 인프라 자체로 정면 대응 — 120대 로봇 플릿 기준 월 ~12PB 저장·7B 모델 1 epoch 학습에 참조 가속기 170~2,566대(표현 방식·데이터 선별에 따라)까지 정량 추산해 실패 에피소드도 원인이 기록되면 자산으로 취급하는 설계<br>⏳ **대기**: arXiv 프리프린트(2026-09, MBZUAI 단독 저자진) `[4]` — 3개 챔버·대시보드가 자체 기관에서 가동 중이라는 스냅샷뿐, 저자 스스로 "RDF 데이터가 정책 성능을 개선한다는 실증은 향후 과제"라 명시. 타 기관 채택·외부 재현 없음. ⚠️ 이번 실행 환경 egress 제한으로 curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | 타 기관 federated 채택 + 학습 성능 개선 실증 + peer-review |
| **[Gemini Robotics 2 / ER 2](https://deepmind.google/blog/gemini-robotics-2-brings-whole-body-intelligence-to-robots/)** (Google DeepMind 전신 제어 VLA + 임바디드 추론 "브레인" 모델) | 🟢 GA(API)/🔵(실배포) | ✨ **주목**: 기존 Gemini Robotics(상체 중심 제어)를 넘어 보행·균형·크라우칭까지 포함한 전신 제어를 단일 VLA(Gemini Robotics 2)로 통합하고, 실시간 영상 이해·멀티로봇 협업이 가능한 임바디드 추론 모델(ER 2)을 Gemini API·AI Studio로 공개 — GR00T·π0 계열과 경쟁하는 빅테크발 whole-body VLA 축으로, Radar 경쟁사 휴머노이드 관찰 목록에 Google이 구체 모델로 처음 추가됨<br>⏳ **대기**: Google DeepMind 공식 발표(2026-07-30, deepmind.google) `[4]` — Safety Instruction Following·Human Proximity 등 자체 벤치마크 위주, 독립 재현·특정 파트너 로봇 실배포 사례 공개 없음. ⚠️ 이번 실행 환경 egress 제한으로 deepmind.google curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | 독립 벤치마크 검증 + 파트너 로봇 실배포 사례 공개 |
