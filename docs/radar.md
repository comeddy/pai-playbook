...
## 🆕 최신 스캔 유입 (2026-09-15 · 1차 검증 완료 2026-07-21)
...
| **[Unitree UnifoLM-X2-1.0](https://x.com/UnitreeRobotics/status/2096932273602048258)** (실시간 월드모델 기반 완전 자율 휴머노이드 복싱 데모) | ⚪ Hype·로드맵 | ✨ **주목**: Unitree가 G1 휴머노이드로 원격조작 없이 완전 자율로 복싱 대결(스탠스 전환·슬립·스트레이트 펀치·바디킥)을 수행하는 실시간 월드모델 기반 계획·의사결정 시스템을 시연 — 2025-09 공개한 오픈소스 프레임워크 UnifoLM-WMA-0의 실전판, 월드모델이 고동역학 상호작용을 실시간 구동한 초기 사례로 화제<br>⏳ **대기**: Unitree 공식 발표(2026-09-07, 공식 X 계정) `[4]` — 단발성 큐레이션 데모(복싱 매치 1건)뿐, 기술 리포트·논문 미공개, 독립 검증·범용 태스크(격투 외) 성능·안전성 데이터 없음. ⚠️ 이번 실행 환경 egress 제한으로 curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | 기술 리포트 공개 + 범용 태스크 성능·독립 검증 |
