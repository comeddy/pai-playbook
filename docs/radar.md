...
## 🆕 최신 스캔 유입 (2026-08-19 · 1차 검증 완료 2026-07-21)
...
| **[Ψ₀ (Psi-Zero)](https://github.com/physical-superintelligence-lab/Psi0)** (사람 영상 사전학습 + 소량 실로봇 데이터로 학습하는 휴머노이드 이동+조작 통합 파운데이션 모델) | 🔵 Research | ✨ **주목**: USC Physical Superintelligence Lab(+Stanford) 발 오픈 모델 — 사람 1인칭 영상 약 800시간 사전학습 후 실로봇 데이터 단 30시간만 파인튜닝해 기존 SOTA 대비 40%+ 높은 성공률(데이터 10배+ 절감) 자체 주장, 코드·가중치·데이터셋까지 실공개<br>⏳ **대기**: GitHub 공식 저장소 확인(Apache-2.0, RSS 2026 accepted 표기, arXiv 2603.12263) `[4]` — 자체 벤치 결과, 독립 재현·peer-review 검증 없음, 실로봇 프로덕션 배포 사례 0 | 독립 벤치마크 재현 + 실배포 사례 |
