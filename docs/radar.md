...
## 🆕 최신 스캔 유입 (2026-09-10 · 1차 검증 완료 2026-07-21)
...
| **[AgiBot World 2026](https://github.com/OpenDriveLab/AgiBot-World)** (5단계 순차 공개 예정인 대규모 실환경 로봇 데이터셋) | 🔵 Research | ✨ **주목**: 중국 AgiBot이 AgiBot G2 로봇으로 상업공간·가정 등 실환경에서 수집한 데이터를 모방학습→강화학습→월드모델 등 5개 연구 축에 맞춰 순차 오픈소스 공개 — 실패-복구 궤적까지 주석 처리한 로봇 데이터 파이프라인 레퍼런스 사례(IROS 2025 최우수논문 파이널리스트·IEEE TRO 2026 게재 AgiBot-World 후속)<br>⏳ **대기**: GitHub 공식 저장소(`OpenDriveLab/AgiBot-World`, 코드 Apache-2.0) `[4]` — 1단계(모방학습)만 공개, 데이터셋 자체 라이선스는 CC BY-NC-SA 4.0으로 상업 이용 제한. 나머지 4단계 미공개, 정량 규모·독립 검증 없음 | 5단계 전체 공개 + 상업 라이선스 조건 확정 |
