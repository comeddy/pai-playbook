...
## 🆕 최신 스캔 유입 (2026-09-16 · 1차 검증 완료 2026-07-21)
...
| **[DWMP](https://arxiv.org/abs/2609.12347)** (신체 동역학 모델과 시각 기반 월드모델을 결합한 이중 구조 휴머노이드 보행 정책) | 🔵 Research | ✨ **주목**: "월드모델 기반 보행"을 실제로 어떻게 구현할지 보여주는 구체적 아키텍처 제안 — 고유감각(관절·자세) 예측 모델과 깊이영상 기반 월드모델을 결합해 경량 student 정책으로 증류, Unitree G1 실기에서 무작위 장애물 통과율 0.95/0.80/0.70(증류 베이스라인 0.80/0.60/0.65 대비 우위) 확보 — Radar의 DreamZero/GR00T N2/Genie 3 월드모델 축과는 다른 각도(학계 아키텍처 제안)<br>⏳ **대기**: arXiv 프리프린트(2026-09-11) `[4]` — 단일 논문, 기관 소속 명시 없음, ablation 신뢰구간 등 세부 부족, 독립 검증 없음. ⚠️ 이번 실행 환경 egress 제한으로 curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | peer-review + 독립 재현 + 기관 소속 확인 |
...
| **[XPENG IRON 휴머노이드 양산 라인](https://www.xpeng.com/pressroom/news/01a080371029a057bc8e8a02a2c6012b)** (자동차 등급 자동화 생산라인, 76DOF·자체 Turing AI 칩 3기 탑재) | ⚪ 로드맵 | ✨ **주목**: 중국 EV 업체 XPENG이 광저우에 휴머노이드 전용 생산라인을 가동해 IRON이 라인에서 자율 보행으로 걸어나오는 시연에 성공 — 완성차 품질관리 체계를 로봇 양산에 그대로 적용한 사례로, Tesla Optimus 양산 지연 국면과 대비되는 경쟁 하드웨어 트랙(현대·BD Atlas·UBTech와 다른 각도: 완성차 업체발 자동화 양산 라인)<br>⏳ **대기**: XPENG 공식 발표(2026-09-08, xpeng.com pressroom) `[4]` — 생산라인 핵심 공정 80%+ 자동화·1대 시연 성공뿐, 연말 목표인 실제 양산은 미시작(중국 내 출시·해외 배송은 2027년 목표). 자율성능·독립검증 없음. ⚠️ 이번 실행 환경 egress 제한으로 curl 200 수동 검증 미수행(커밋 메시지·이슈 참조) | 연말 양산 개시 확인 + 자율성능 독립 검증 |
