...
## 🆕 최신 스캔 유입 (2026-08-11 · 1차 검증 완료 2026-07-21)
...
| **[Ego2Robot](https://arxiv.org/abs/2608.02580)** (사람 1인칭 시점 영상→로봇 학습 데이터 합성 파이프라인) | 🔵 Research | ✨ **주목**: 사람 손 동작을 15개 로봇 embodiment로 변환·합성해 18,561시간 규모(사상 최대 주장)의 ego-to-robot 데이터를 생성 — 실기(ARX ACone) 파인튜닝에서 성공률 개선까지 검증, SiMDex(마이닝 방식)와 달리 합성(변환+렌더링) 접근<br>⏳ **대기**: arXiv 2608.02580(2026-08-03, Alibaba Qwen Team·인민대·상하이테크 등) `[4]` — RoboTwin2.0 변형 벤치·π0.5 비교 자체 결과, 5개 태스크 소규모 실기 검증뿐, 독립 재현 없음. 데이터셋·코드 공개 여부 불명(프로젝트 페이지만 언급) | peer-review + 데이터셋 공개 확인 + 대규모 독립 재현 |
