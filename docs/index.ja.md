...
!!! info "非公式資料（Unofficial）"
    本サイトは個人が運営する参照資産であり、**AWS（Amazon Web Services）の公式ドキュメント・公式見解ではありません。** サービス仕様・価格・リージョン対応は必ず [AWS 公式ドキュメント](https://docs.aws.amazon.com/)でご確認ください。サードパーティ製品・技術の成熟度判定は、各項目のソース等級に基づく運営者個人の評価です。
