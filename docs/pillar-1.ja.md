...
- **WFM ベースのデータ増幅 — GR00T-Dreams/DreamGen** `[1]/[3]`: 少数の実デモから「dream」軌跡を生成して学習データを増幅する経路です。NVIDIA は GR00T N1→N1.5 の更新に必要なデータ確保を **手動テレオペレーション ~3か月 → 約36時間** に短縮したと発表しました（[DreamGen, arXiv:2505.12705](https://arxiv.org/abs/2505.12705)）。⚠️ 時間短縮値はベンダー自身の数値 — 方向性の指標としてのみ引用。
...
---
_owner: Youngjin · updated: 2026-09 · volatility: 中（データセットのバージョン・サイズは折りたたみブロックで高）· sources: [1] 公式/論文, [3] ベンダーブログ, [4] 未検証_
