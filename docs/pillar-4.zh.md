...
_最终更新: 2026-09 · owner: Youngjin · volatility: 中（边缘 HW·模型为高）_
...
- **deploy-side gap —— 部署失败的多数不是物理，而是"接线"** `[2]`: 与训练侧 gap（物理·渲染不匹配）不同，把训练好的策略在真机上执行这一阶段的失败，多数来自**观测 layout·执行缩放不匹配**。例: Unitree G1 whole-body 控制要求观测 86 项×6 tick=516 维的排列顺序以及 `action_scale=0.25` 这类常数与仿真完全一致（一致时 0.5m/s 指令下 ~0.38m/s 稳定行走，不一致时无法行走）。这是策略移植检查清单的第一位 —— 在讨论 DR·SysID 之前先查这个。
...
**客户需求/问题**: "要在韩国工厂安装机器人单元，法律上需要配备什么？协作机器人是不是不用围栏？"

**解决方案概览** `[1]`:

- **国际标准地图**: [ISO 10218-1:2025](https://www.iso.org/standard/73933.html)（机器人本体）· ISO 10218-2:2025（机器人单元·集成）· ISO/TS 15066:2016（协作机器人）· IEC 61496-2/-3（光幕[^aopd]/安全激光扫描仪）· ISO 12100（风险评估）· ANSI/RIA R15.06（美国）。
- **协作机器人四种安全运行模式**（ISO/TS 15066）: ① 安全额定监控停止（safety-rated monitored stop）② 手动引导（hand guiding）③ 速度与间距监控（speed & separation monitoring）④ 功率与力限制（power & force limiting）。要与人共享空间，必须用**经认证的传感器·设备实现并验证**其中之一。**2025 年修订中，ISO/TS 15066 的接触力·压力限值被并入 ISO 10218-2 正文**。
- **韩国的法定组合** —— [《产业安全保健基准规则》第223条](https://www.law.go.kr/법령/산업안전보건기준에관한규칙): 为防止产业机器人运行中的劳动者危险，原则上要求设置**高度 1.8m 以上的围栏（栅栏）**；无法设围栏的开口·进入区段，须用安全垫或光电式防护装置（光幕）等感应型防护装置阻断接触。此时防护装置必须是依据[《产业安全保健法》第84条](https://www.law.go.kr/법령/산업안전보건법)的 **KCs 强制安全认证品**（光幕=对应 IEC 61496-2，激光扫描仪=对应 IEC 61496-3，雇佣劳动部《防护装置安全认证告示》）。即在韩国，机器人作业区域事实上的法定组合是**「1.8m 围栏 +（开口处）KCs 认证光幕/安全垫」**。⚠️ 准确的条文·款项以国家法令信息中心原文为最终确认。
- **成本直觉** `[4]`: 安全激光扫描仪每台数千美元级，安全围栏每米大约 $60–120（厂商·规格差异大的估计值）+ 风险评估·认证交付周期。防护配置是机器人本体之外的隐性成本。

**AWS 映射**: 无直接映射（法规在 AWS 之外）—— 但这份法规负担是 [pillar-3 仿真经济学](pillar-3.md)（"仿真里没有围栏·认证·事故"）与 [pillar-5 分层防御](pillar-5.md)（智能体层 Policy + 机器人层 ISO 确定性安全）的前提。

**决策标准**: "协作机器人所以不用围栏"不是自动成立的 —— **风险评估（ISO 12100）的结果决定四种模式中实现哪一种、用什么认证设备**。韩国安装咨询应连接到条文确认 + 韩国机器人产业振兴院（KIRIA）产业机器人安全手册。

**客户案例**: （法规合规不是案例，而是部署的前提条件）

**➡️ 后续行动**: 建议在真机 PoC 提案书中**从一开始就把防护配置成本·KCs 认证交付周期列为单独行项** —— 若事后才发现，日程会整体推迟。在同一页幻灯片中提议"仿真先行验证"（→ [pillar-3](pillar-3.md)），说服力就完整了。

**🔗 相关资产**: [pillar-3 为什么选择仿真](pillar-3.md) · [pillar-5 安全 & 护栏](pillar-5.md)
...
---
_owner: Youngjin · updated: 2026-09 · volatility: 中（边缘 HW·厂商指标为高）· sources: [1] 官方/论文, [3] 厂商/PR, [4] 未经验证。2026 arXiv 预印本为非评审(illustrative)。_
