...
- `owner`: **始终只有 1 人**（责任单一化原则）。若未定则标为 `待定 ⚠️`，**作为债务保留**（不隐藏）。
- `验证`（可选）: 参与一手来源验证（对照官方发布·论文原文·许可证）的人 —— **可多人**，用逗号列出。与 owner 相同时可省略。
...
**术语脚注惯例（持续）**: 纳入或更新内容时，SA 难以立即理解的术语须在同一次变更中用 `[^标签]` 脚注一并处理 —— 已有标签则复用；新术语则在页面最底部 `<!-- 용어 각주 -->` 块中按"**术语** — 1~2 句说明"格式追加。如有经验证的官方视频，在定义末尾附 🎥 链接（合并前通过 oEmbed[^oembed] 响应核对标题·频道）。标记只放在正文首次出现处，禁止用于标题和 mermaid 块。标签是机械标识符，绝不翻译；四种语言完全一致地应用（标签·URL 相同）—— 详细规则见 `i18n/glossary.md`。
...
[^staleness]: **staleness（新鲜度下降）** — 文档·数据自上次复核以来变旧、与实际逐渐脱节的程度。本 playbook 按页面的变动性等级设定允许期限（1/3/6 个月），一旦超过就自动附加徽章。
[^oembed]: **oEmbed** — 只凭一个 URL 就以 JSON 返回该内容的标题·作者等嵌入信息的标准 API。用于在发布前机械地验证视频链接是否实际存在、标题·频道是否相符。
