...
**术语脚注惯例**: 纳入或更新内容时，SA 难以立即理解的术语须在同一次变更中用 `[^术语]` 脚注一并处理 —— 已有脚注 id 则复用；新术语则在页面底部 `<!-- 용어 각주 -->` 块中按"**术语** —— 1~2 句说明"格式追加（如有有用的官方视频则附 🎥 链接，并经 oEmbed 验证其存在）。标记只放在正文首次出现处，禁止用于标题和 mermaid 块。四种语言完全一致地应用（标记 id 相同、URL 相同）。
