...
## 🆕 最新スキャン流入（2026-09-14 · 一次検証完了 2026-07-21）
...
| **[Skild AI S1](https://skild.ai/blogs/s1)**（人間の実演映像 1 本だけをもとに、ファインチューニングなしで最長 10 分の長時間タスクを実行するインコンテキスト学習型ロボット基盤モデル） | 🟡 Preview | ✨ **注目**: Skild AI は人間の実演映像 1 本を「視覚プロンプト」として受け取り、ファインチューニングや重みの変更なしに最長 10 分・数十ステップの未学習の長時間タスク（植木鉢の植え付け・パンケーキ調理・ハンドドリップコーヒー・キット組み立て）を成功率 66%（ゼロショットは 9%）で実行 —— Generalist AI GEN-1.5 と同じ「ファインチューニング不要のワンショット/インコンテキスト学習」軸のもう一つの主要事例で、「本日より少数の産業パートナーへの展開を開始」と発表<br>⏳ **待機**: Skild AI 公式発表（2026-08-25、skild.ai 公式ブログ）`[4]` —— 自己申告ベンチマーク（未学習タスク 4 件）のみで、論文・重み・API は未公開（非公開モデル）。産業パートナーへの展開は「開始」の発表のみで、実運用の成果・独立検証はなし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 産業パートナーでの実導入成果公開 + 独立ベンチマーク/論文公開 |
