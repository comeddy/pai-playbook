---
ko_hash: b2fbad3923d63414eccb5e66c647deaebffe87fd
---
# Radar — キュー / ウォッチリスト
...
## 🆕 最新スキャン流入（2026-09-10 · 一次検証完了 2026-07-21）
...
| **[AgiBot World 2026](https://github.com/OpenDriveLab/AgiBot-World)**（5 段階に分けて順次オープンソース公開予定の大規模実環境ロボットデータセット） | 🔵 Research | ✨ **注目**: 中国 AgiBot が AgiBot G2 ロボットで商業空間・家庭などの実環境から収集したデータを、模倣学習→強化学習→ワールドモデルなど 5 つの研究軸に沿って順次オープンソース公開 —— 失敗-復帰軌道までアノテーションしたロボットデータパイプラインのリファレンス事例（IROS 2025 最優秀論文ファイナリスト・IEEE TRO 2026 掲載の AgiBot-World の後継）<br>⏳ **待機**: GitHub 公式リポジトリ（`OpenDriveLab/AgiBot-World`、コードは Apache-2.0）`[4]` —— 現時点では第 1 段階（模倣学習）のみ公開、データセット自体のライセンスは CC BY-NC-SA 4.0 で商用利用制限あり。残り 4 段階は未公開、定量規模・独立検証なし | 5 段階すべての公開 + 商用ライセンス条件の確定 |
