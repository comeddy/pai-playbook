---
ko_hash: ccd455c8d6d1b44aa69b322b75ba6416b1cd5825
---
# Radar — キュー / ウォッチリスト
...
## 🆕 最新スキャン流入（2026-09-15 · 一次検証完了 2026-07-21）
...
| **[Unitree UnifoLM-X2-1.0](https://x.com/UnitreeRobotics/status/2096932273602048258)**（リアルタイム世界モデル駆動の完全自律ヒューマノイド・ボクシングデモ） | ⚪ Hype・ロードマップ | ✨ **注目**: Unitree が G1 ヒューマノイドで遠隔操作なしに完全自律でボクシング対戦（構え変え・スリップ・ストレートパンチ・ボディキック）を実演 —— リアルタイム世界モデルによる計画・意思決定システムが駆動、2025-09 公開のオープンソースフレームワーク UnifoLM-WMA-0 の実戦版で、世界モデルが高ダイナミクスな相互作用をリアルタイムで駆動した初期事例として話題<br>⏳ **待機**: Unitree 公式発表（2026-09-07、公式 X アカウント）`[4]` —— 単発の演出済みデモ（ボクシング1試合のみ）で、技術レポート・論文は未公開、格闘以外の汎用タスク性能・安全性データの独立検証はなし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 技術レポート公開 + 汎用タスク性能・独立検証 |
