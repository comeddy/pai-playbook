...
## 🆕 最新スキャン流入（2026-09-26 · 一次検証完了 2026-07-21）
...
| **[XPENG IRON](https://www.xpeng.com/news/01a080371029a057bc8e8a02a2c6012b)**（中国 EV メーカー XPENG のヒューマノイド、自動車級の量産ラインで完成機が自力歩行してライン外へ） | ⚪ ロードマップ | ✨ **注目**: 完成車メーカーが自社 EV 生産のノウハウ（工程自動化率 80%以上）をそのままヒューマノイド量産ラインに移植 —— いまだ「学習・データ収集」段階にとどまる Tesla Optimus とは対照的に、実機が自力でラインから歩き出す様子を公開（76 自由度、片手あたり 21 自由度）—— 韓国の顧客対話に「中国 EV 発ヒューマノイド」という新たな競合トラックを追加<br>⏳ **待機**: XPENG 公式発表（2026-09-07/08、xpeng.com）+ CnEVPost・Electrek による交差確認 `[4]` —— 「量産開始」は 2026 年末の目標にすぎず、初期配備は自社店舗・キャンパス向け、海外・一般商業向け配送は 2027 年からの予定。独立した性能・安全検証はなし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 実際の量産出荷開始 + 第三者による安全・性能検証 |
