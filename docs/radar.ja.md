---
ko_hash: 6c657faa2c591ec91c1181f248a47b526104685d
---
# Radar — キュー / ウォッチリスト
...
## 🆕 最新スキャン流入（2026-09-25 · 一次検証完了 2026-07-21）
...
| **[Opt2VLA](https://opt2vla.github.io/)**（接触が多いヒューマノイドの全身操作向けの力（force）認識 VLA —— 幾何的な動作目標に加えて接触力の基準値まで予測） | 🔵 Research | ✨ **注目**: 従来の VLA は動作目標のみを予測し、接触後に視覚情報が不安定になる瞬間の力制御は全身コントローラーに委ねていた —— Opt2VLA は単一のマルチタスク VLA が動作目標と接触力の基準値を同時に予測し、タスクごとの RL 全身コントローラーがその力を追従する。軌道最適化（TO）により接触と整合的な学習データを直接生成する。TANGO（全身ナビゲーション）とは異なる角度から「全身制御型 VLA」の操作（マニピュレーション）軸を補完する研究<br>⏳ **待機**: arXiv プレプリント（2026-09-21、Georgia Tech・Ye Zhao/Zsolt Kira 研究室など）`[4]` —— 接触が多いタスク 3 種の自己申告ベンチマークのみで、peer-review・独立再現・オープンソース公開なし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | peer-review + コード/チェックポイントのオープンソース公開後の独立再現 |
