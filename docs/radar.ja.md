...
## 🆕 最新スキャン流入（2026-09-24 · 一次検証完了 2026-07-21）
...
| **[Figure Index → Helix 2.5](https://www.figure.ai/news/helix-2-5-zero-shot-30-home-generalization)**（クラウドソーシングの人間映像データで事前学習したヒューマノイド向けニューラルネットワーク、未学習の家庭 30 軒でゼロショット検証） | 🟡 Preview | ✨ **注目**: Figure は Index で事前学習した単一のポリシーを、データ収集やファインチューニングなしにベイエリアの実際の家庭 30 軒（すべて未学習）へ展開し、リビングの片付け・タオルの折り畳み・ベッドメイキングの 3 タスクを実行 —— Index 事前学習だけでゼロショットの全体タスク成功率が 9%→56%（420 回中 237 回）に上昇したと発表、クラウドソーシングデータパイプラインが実際のポリシー性能に結びつくことを示す初の定量的な証拠で、この流入項目の昇格条件のうち「実測公開」を満たした<br>⏳ **待機**: Figure 公式発表（2026-09-17、figure.ai）`[4]` —— 自己測定値のみ（9%→56%、30 軒、420 回試行）で、独立再現・第三者検証なし。Index アプリ自体の報酬支払い（1,500 万ドル）・品質管理方式も依然自己申告レベル。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 独立再現・第三者ベンチマーク検証 + 対象家庭・タスクを広げた追加実証 |
...
| **[TANGO](https://arxiv.org/abs/2609.09158)**（関節空間アクションを直接予測する全身制御型ヒューマノイド・ナビゲーション VLA） | 🔵 Research | ✨ **注目**: 2D ウェイポイントではなく 29 自由度の関節空間アクションを直接予測し、障害物が多い室内で身をかがめる・横に避ける・跨ぐといった動作を言語指示に応じて自ら判断する whole-body VLA —— 独自のシミュレーションデータパイプライン（Plan-Edit-Track、6.46 万トラジェクトリ）で学習し、Unitree G1 でのゼロショット sim-to-real[^s2r] 転移に成功 —— VLA と sim-to-real の両軸を同時に扱う学術事例<br>⏳ **待機**: arXiv プレプリント（2026-09-08、UC Berkeley・Peking Univ・Tsinghua・HKU・Princeton 共同）`[4]` —— 自己申告ベンチマーク（成功率・SPL・衝突率）のみで、peer-review・独立再現なし。データパイプライン・モデル・チェックポイントはオープンソース化予定だが未公開。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | peer-review + オープンソース公開後の独立再現 |
| **[Skild AI S1](https://skild.ai/blogs/s1)**（人間の実演映像 1 本だけをもとに、ファインチューニングなしで最長 10 分の長時間タスクを実行するインコンテキスト学習型ロボット基盤モデル） | 🟡 Preview | ✨ **注目**: Skild AI は人間の実演映像 1 本を「視覚プロンプト」として受け取り、ファインチューニングや重みの変更なしに未学習の長時間タスクを実行 —— [NVIDIA 公式ブログ](https://blogs.nvidia.com/blog/skild-ai-s1-physical-ai/)（2026-09-10）が Skild・NVIDIA・Foxconn による NVIDIA Blackwell システム組立ライン（busbar・limit block の取り付け、ネジ 16 本の締結など）での実導入、有償顧客 60 社超、初回商用導入からわずか 10 か月での年間経常収益（ARR）1 億ドル達成を公式に確認 —— 「少数パートナーへの展開開始」の宣言段階から、実際の産業収益段階へ進んだ事例として更新<br>⏳ **待機**: NVIDIA・Skild AI 公式発表（2026-09-10）`[4]` —— 売上・顧客数は自社発表値で、第三者監査・独立性能検証はなし（汎用タスク成功率などの詳細は非公開）。AWS マッピング・ソウルリージョン連携事例はなし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 独立性能・収益検証（第三者監査）+ AWS マッピング事例の公開 |
| **[Strands Robots](https://strandsagents.com/blog/robots-working-together-model-hardware-standard-strands-robots/)**（AWS のオープンソースエージェントフレームワーク Strands Agents による、ロボット・物理ハードウェアの自然言語制御拡張。Zenoh メッシュネットワーク + AWS IoT Core によるフリート連携を備える） | 🟡 Preview | ✨ **注目**: AWS 自社のオープンソースエージェントフレームワーク Strands Agents が、自然言語でロボットを直接制御できる拡張を公開 —— `Robot()`/`Simulation()` インスタンスは自動的に Zenoh メッシュのピアとなりローカルネットワーク上で互いを発見・制御し、AWS IoT Core で地理的に分散したフリートまで連携、既存の ROS 2 グラフもそのまま観測・命令可能 —— Radar 🔗 エージェント/接続軸（MCP for robotics、AgentCore Payments/Registry）に AWS 自社のロボティクスエージェント事例が初めて加わった<br>⏳ **待機**: strands-labs/robots オープンソースリポジトリ（2026-08、v0.5.x）+ strandsagents.com 公式ブログ `[4]` —— docs.aws.amazon.com 上で GA サービスとして文書化されていないコミュニティ/OSS 段階で、同時発表の「Model Hardware Standard」は限定リサーチプレビュー。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | AWS 公式サービスとしての GA 文書化 + 実際のロボットフリートでの本番事例公開 |
| **[OpenAI がヒューマノイドロボット開発を確認](https://sources.news/p/introducing-the-sources-podcast-with)**（Sam Altman が Sources ポッドキャストで自社のヒューマノイドロボット開発を直接確認） | ⚪ Hype・ロードマップ | ✨ **注目**: OpenAI の CEO が「我々は間違いなくヒューマノイドをやる」と初めて直接明言 —— ロボットハードウェアという形態への正式な事業拡大を示唆し、Radar の競合ヒューマノイド観察リスト（Tesla/Figure/1X/Google）に OpenAI が新たに加わる契機となり、顧客対話での話題性が最も高い項目のひとつになりうる<br>⏳ **待機**: 2026-09-01 の Sources ポッドキャストでの発言のみ `[4]` —— プロトタイプ・発売スケジュール・製造パートナーは一切非公開（Forbes などで確認済み）、データセンター向けなど「他の形態」にも同時に言及しており、ヒューマノイド専用戦略かどうかも不明確。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | プロトタイプ・製造パートナー・発売スケジュールの公式発表 |
| **[Intrinsic Core](https://www.intrinsic.ai/blog/posts/introducing-intrinsic-core)**（Alphabet のロボティクス子会社 Intrinsic が公開した産業用ロボティクススタックのオープンソース化 —— リアルタイム制御・NVIDIA FoundationPose ベースの姿勢推定・モーション/グラスプ計画・シミュレーション・カメラキャリブレーション・ROS 互換ドライバ） | 🟢 GA（オープンソース） | ✨ **注目**: Intrinsic が自社の実運用デプロイで使っているスタックそのものを、ROSCon 2026（トロント）で Apache 2.0 ライセンスの下に一括公開 —— Isaac 生態系が一強の状況に Alphabet 系の代替スタックが初めて登場し、Radar 🖥️ シミュレーション/ツール軸（Isaac/MuJoCo/Newton）に新たな観察対象が加わった<br>⏳ **待機**: Intrinsic 公式ブログ + The Robot Report（2026-09-22/23）`[4]` —— コード自体は GitHub で公開済み（Apache 2.0）だが、Intrinsic 以外での独立した採用事例や AWS マッピング事例はまだない。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 独立した採用事例の公開 + AWS インフラマッピングの検証 |
| **[Agility Robotics Digit 5](https://www.agilityrobotics.com/content/agility-unveils-digit-5-humanoid-robot-built-for-cooperatively-safe-work-at-scale)**（既存の Digit@GXO に近接協働安全アーキテクチャと 90 分ランタイムのバッテリーを加えた第 5 世代の汎用ヒューマノイド） | 🟡 Preview | ✨ **注目**: Radar の pillar-4 で「最も検証が進んだ有償ヒューマノイド作業」として既に掲載されている Agility Digit（@GXO）の次世代モデル —— 衝突リスクを検知すると停止・着座し、視覚・音声で警告を発することで人と近接して作業できる安全設計を備え、3 億ドル超の複数年注文を確保した上で、初めて EU・英国への商用展開拡大を発表（CE マーキング取得計画）<br>⏳ **待機**: Agility 公式発表（2026-09-15、agilityrobotics.com）`[4]` —— 注文残高と CE マーキング計画にとどまり、Digit 5 自体の実運用事例はまだない（既存の GXO 事例は前世代の Digit）。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | Digit 5 の実運用事例公開（EU/北米）+ 安全アーキテクチャの第三者検証 |
