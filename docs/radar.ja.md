...
## 🆕 最新スキャン流入（2026-09-21 · 一次検証完了 2026-07-21）
...
| **[WholeBodyWAM](https://arxiv.org/abs/2609.16644)**（事前学習済みワールドアクションモデルを whole-body controller に基づく全身制御へ拡張するヒューマノイド loco-manipulation フレームワーク） | 🔵 Research | ✨ **注目**: マニピュレーション中心に事前学習されたワールドアクションモデル（WAM）の知識を保持しつつ、異種の全身制御器（whole-body controller）のセマンティクスを接地させ、歩行まで含む全身協調 loco-manipulation へ汎化 —— シミュレーション総合成功率 91.9%で、Radar 🔬 ワールドアクションモデル軸（DreamZero→GR00T N2）をマニピュレーションから全身制御へ拡張する最新の学術事例<br>⏳ **待機**: arXiv プレプリント（2026-09-17）`[4]` —— 自己申告のシミュレーションベンチマークのみで、実機検証・peer-review・独立再現なし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 実機（sim-to-real）検証 + peer-review + 独立再現 |
