---
ko_hash: f6746ec27cffa9b0eecbad11c1e382fc17d694db
---
# Radar — キュー / ウォッチリスト
...
## 🆕 最新スキャン流入（2026-08-15 · 一次検証完了 2026-07-21）
...
| **[In-Context VLA / VLA-Talker](https://arxiv.org/abs/2608.05738)**（エージェンティックなツール利用[^agent]で推論-制御の衝突を回避する VLA[^vla] 構造） | 🔵 Research | ✨ **注目**: VLA に自由形式のテキスト推論（CoT）を入れると遅延や最適化目標の衝突で低レベル制御性能が落ちることを実証し、代わりにポリシーがオープンボキャブラリ検出器・単眼深度推定・VLM を能動的に呼び出して必要な情報だけ取得する構造（VLA-Talker）へ転換 —— 推論と行動をテキストで混ぜずアーキテクチャで分離するアプローチ<br>⏳ **待機**: arXiv 2608.05738（2026-08-06、単一研究チーム）`[4]` —— 自己申告ベンチマークのみ、独立再現・peer-review なし、コード・重みの公開状況は不明 | peer-review + 独立再現 + 公開の確認 |
