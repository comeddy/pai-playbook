---
ko_hash: 9134d2008f0de2b4003a1c04055f29987b185831
---
# Radar — キュー / ウォッチリスト
...
## 🆕 最新スキャン流入（2026-09-20 · 一次検証完了 2026-07-21）
...
| **[GE-Act 2.0](https://arxiv.org/abs/2609.05588)**（AgiBot Genie チームがゼロから事前学習したワールドアクションモデル） | 🔵 Research | ✨ **注目**: 既存のワールドアクションモデル（WAM）が事前学習済みの映像生成モデルをそのまま引き継ぐのに対し、生成・アクションコンポーネントをマニピュレーションデータでゼロから学習 —— Radar のワールドアクションモデル軸（DreamZero→GR00T N2、Genie 3）に中国 AgiBot 発のオープン研究事例を追加<br>⏳ **待機**: arXiv プレプリント（2026-09-04、AgiBot Genie Team・NUS LV-Lab・BUAA 共同）`[4]` —— 自己申告ベンチマーク（シム・実機マニピュレーション成功率）のみで、peer-review・独立再現なし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | peer-review + 独立再現 + オープンソース公開 |
...
| **[Robot Data Factory (RDF)](https://arxiv.org/abs/2609.16705)**（mission→task→skill→episode→dataset→benchmark という階層でロボットの「経験」の収集・検証を標準化するインフラ、MBZUAI） | 🔵 Research | ✨ **注目**: Radar 流入項目として初めて、モデルやパートナーシップではなくデータインフラそのもので「物理 AI データパイプライン」軸に正面から対応 —— 参照構成として 120 台のロボットフリートで月間約 12PB の保持ストレージ、7B モデルを 1 epoch 学習するには表現方法・データ選別によって 170〜2,566 台の参照アクセラレータが必要と定量的に推算し、失敗原因が記録されていれば失敗エピソードも資産として扱う設計<br>⏳ **待機**: arXiv プレプリント（2026-09、MBZUAI 単独の著者陣）`[4]` —— 自機関内で 3 つのチャンバーとダッシュボードが稼働しているというスナップショットのみで、著者自身が「RDF データがポリシー性能を改善するという実証は今後の課題」と明言。他機関での採用・外部再現はなし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 他機関での federated 採用 + 学習性能改善の実証 + peer-review |
| **[Gemini Robotics 2 / ER 2](https://deepmind.google/blog/gemini-robotics-2-brings-whole-body-intelligence-to-robots/)**（Google DeepMind の全身制御 VLA + エンボディード推論「ブレイン」モデル） | 🟢 GA（API）/🔵（実デプロイ） | ✨ **注目**: 上半身中心だった従来の Gemini Robotics を超えて、歩行・バランス・しゃがみ込みまで含む全身制御を単一の VLA（Gemini Robotics 2）に統合し、リアルタイム映像理解・マルチロボット協調が可能なエンボディード推論「ブレイン」モデル（ER 2）を Gemini API・AI Studio で公開 —— GR00T・π0 系列と競合するビッグテック発の whole-body VLA 軸で、Radar の競合ヒューマノイド観察リストに Google が具体的なモデルとして初めて加わった<br>⏳ **待機**: Google DeepMind 公式発表（2026-07-30、deepmind.google）`[4]` —— Safety Instruction Following・Human Proximity など自己申告ベンチマークが中心で、独立再現や特定パートナーロボットでの実デプロイ事例の公開はなし。⚠️ 本実行環境の egress 制限により deepmind.google の curl 200 手動確認は未実施（コミットメッセージ・issue 参照） | 独立ベンチマーク検証 + パートナーロボットでの実デプロイ事例公開 |
