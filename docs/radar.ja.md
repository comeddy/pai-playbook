---
ko_hash: a72aca50427a05ff74844f48353567b597044e41
---
# Radar — キュー / ウォッチリスト
...
## 🆕 最新スキャン流入（2026-09-16 · 一次検証完了 2026-07-21）
...
| **[DWMP](https://arxiv.org/abs/2609.12347)**（身体動力学モデルと視覚ベースの世界モデルを組み合わせた二重構造のヒューマノイド歩行ポリシー） | 🔵 Research | ✨ **注目**: 「世界モデル駆動の歩行」を実際にどう実装するかを示す具体的なアーキテクチャ提案 —— 固有感覚（関節・姿勢）予測モデルと深度映像ベースの世界モデルを組み合わせ、軽量な student ポリシーへ蒸留。Unitree G1 実機でランダムな障害物コースをテストし、通過率 0.95/0.80/0.70（蒸留ベースラインの 0.80/0.60/0.65 を上回る）を確保 —— Radar の DreamZero・GR00T N2・Genie 3 の世界モデル軸とは異なる角度（学術的アーキテクチャ提案）<br>⏳ **待機**: arXiv プレプリント（2026-09-11）`[4]` —— 単一論文で所属機関の明記なし、アブレーション（信頼区間など）の詳細が不足、独立検証なし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | peer-review + 独立再現 + 所属機関の確認 |
...
| **[XPENG IRON ヒューマノイド量産ライン](https://www.xpeng.com/pressroom/news/01a080371029a057bc8e8a02a2c6012b)**（自動車グレードの自動化生産ライン、76 DOF・自社製 Turing AI チップ 3 基搭載） | ⚪ ロードマップ | ✨ **注目**: 中国 EV メーカー XPENG が広州にヒューマノイド専用生産ラインを稼働させ、IRON がライン上で自律歩行して出てくる実演に成功 —— 完成車の品質管理体系をロボット量産にそのまま適用した事例で、Tesla Optimus の量産停滞局面と対照的な競合ハードウェアトラック（現代・BD Atlas、UBTech とは異なる角度：完成車メーカー主導の自動化量産ライン）<br>⏳ **待機**: XPENG 公式発表（2026-09-08、xpeng.com pressroom）`[4]` —— 生産ライン中核工程の 80%以上の自動化・1 台の実演成功のみで、年末目標の実際の量産は未開始（中国国内発売・海外配送は 2027 年目標）。自律性能・独立検証はなし。⚠️ 本実行環境の egress 制限により curl 200 の手動確認は未実施（コミットメッセージ・issue 参照） | 年末量産開始の確認 + 自律性能の独立検証 |
