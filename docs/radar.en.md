...
## 🆕 Latest scan intake (2026-07-27 · primary verification completed 2026-07-21)
...
| **Anthropic × Physical Intelligence acquisition rumor** | ⚪ Hype/roadmap | Social rumor (Scoble tweet) broke 2026-07-19 → The Information/TechCrunch reported "acquisition talks did happen in spring 2026," but no actual acquisition; PI CEO Karol Hausman denied it internally on Slack `[4]` — secondary reporting only, no primary confirmation from either party (re-confirmed by TechCrunch 2026-07-21, no further movement). PI runs on GCP (see pillar-2) and is an OpenAI portfolio company, so a deal would matter for the cloud/competitive landscape | Official statement from either party (deal completed or explicitly off) |
...
| **Mistral Robostral Navigate** (8B single-camera navigation model) | 🟡 Preview | Confirmed via Mistral's official blog `[4]` (2026-07-08) — navigates from a single RGB camera plus a natural-language instruction alone (no depth, no multi-camera rig); 76.6% on R2R-CE (self-reported benchmark, no independent reproduction). Trained entirely in simulation (400k generated trajectories across 6,000 scenes); no real-robot deployment case yet — Mistral's first robotics model (a new entrant to the competitive landscape) | Real-robot sim-to-real validation + independent benchmark reproduction |
