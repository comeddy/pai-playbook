...
流入项目的一手来源确认（官方发布·论文原文·许可证对照）与晋升判断由**人（owner）**完成。自动扫描可能产生貌似合理的错误（例如不存在的产品世代名），因此未经验证不会晋升。通过后，owner 会以[标准模板](maintenance.md#标准模板)将其编入相应支柱并从 Radar 中移除。完整流程参见[晋升管道](maintenance.md#playbook-晋升管道)。
